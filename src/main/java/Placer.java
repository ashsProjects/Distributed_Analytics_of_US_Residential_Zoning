...
public class Placer {
    public static void calculateZoning() throws IOException {
        SparkSession spark = SparkSession.builder().appName("Placer").master("local[*]").getOrCreate();
        String filePath = "hdfs://salem.cs.colostate.edu:31190/zoning/Placer/placer-parcels.csv";
        ...
        JavaRDD<String> zones = df.filter(line -> !line.equals(header))
            .map(x -> {
                String zoningType = x.getString(33);
                if (zoningType == null) zoningType = "null";
                return zoningType.toLowerCase();
            });
        
        FileWriter writer = new FileWriter("./outputs/placer/countOfAllZonesInPlacer.txt", true);
        ...
        JavaRDD<String> residential = zones.filter(x -> (x.contains("single") || x.contains("home") || x.contains("apartment")));

        Map<String, Long> countResidential = residential.countByValue();
        FileWriter writer2 = new FileWriter("./outputs/placer/countOfAllResidentialInPlacer.txt", true);
        writer2.write("Count of all types of Residential in Placer\n");
        long total = 0;
    ...
    public static void calculateTransport(JavaSparkContext sc) {
        JavaRDD<String> inputFile = sc.textFile("hdfs://salem.cs.colostate.edu:31190/zoning/Placer/transportation_means.csv");
        ...
        meanCommute.saveAsTextFile("./outputs/placer/meanCommute");

        JavaRDD<String> typeFile = sc.textFile("hdfs://salem.cs.colostate.edu:31190/zoning/Placer/transportation_type.csv");
        ...
        commuteTypeByCounty.sortByKey().saveAsTextFile("./outputs/placer/commuteTypes");
    }

    public static void calculatePoverty(JavaSparkContext sc) {
        JavaRDD<String> inputFile = sc.textFile("hdfs://salem.cs.colostate.edu:31190/zoning/Placer/poverty.csv");
        ...
        meanCommuteByCounty.sortByKey().saveAsTextFile("./outputs/placer/percentPoverty");
    }

    public static void calculateOccupancy(JavaSparkContext sc) {
        JavaRDD<String> inputFile = sc.textFile("hdfs://salem.cs.colostate.edu:31190/zoning/Placer/occupancy.csv");
        JavaPairRDD<String, String> meanCommuteByCounty = inputFile.zipWithIndex()
            .filter(pair -> pair._2() >= 3).map(pair -> pair._1()).mapToPair(x -> {
                String[] desc = x.split(",");
                StringBuilder sb = new StringBuilder();
        ...
        meanCommuteByCounty.sortByKey().saveAsTextFile("./outputs/placer/occupancy");
    }
    ...
    public static void calculateQOL(JavaSparkContext sc) {
        ...
        JavaRDD<String> mentalHealth = sc.textFile("hdfs://salem.cs.colostate.edu:31190/zoning/Placer/mental_health.csv")
            .zipWithIndex().filter(pair -> pair._2() >= 2).map(pair -> pair._1())
            .filter(x -> x.startsWith("Placer"));
        ...
        JavaPairRDD<String, List<String>> qualitiesByCounty = allQualities.filter(line -> !line.equals(header))
            .filter(x -> x.startsWith("CAPlacer ")).mapToPair(x -> {
            ...
            allQualitiesPlusHealth.sortByKey().saveAsTextFile("./outputs/placer/allQualitiesPlusHealth");
    }
    ...
    public static void calculateHousingCharacteristics(JavaSparkContext sc) {
        ...
        JavaPairRDD<String, String> racesByCounty = raceFile.zipWithIndex()
            ...
            racesByCounty.sortByKey().saveAsTextFile("./outputs/placer/racesByCounty");
        ...
        JavaPairRDD<String, String> financeByCounty = financialFile.zipWithIndex()
            ...
            financeByCounty.sortByKey().saveAsTextFile("./outputs/placer/financesByCounty");
    }

    public static void main(String[] args) throws IOException {
        calculateZoning();

        SparkConf sparkConf = new SparkConf().setMaster("local").setAppName("Placer Analysis");
